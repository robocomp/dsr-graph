...
void SpecificWorker::compute()
{
	...
	// TEST 5  N threads editing random nodes in the graph M times each using prx
	{
		std::cout << std::endl;
		...
		auto lambda = [this]
		{
			...
			{
				auto ind = dist(gen);
				auto &n = *(graph->getNodePtr(ind).get());
				//auto n = graph->getNode(ind);
				n->type = "morningsinger";
		...
		std::cout << "threads initiated" << std::endl;
		for(auto &t : threads)
			t.join();
		std::cout << "threads finished" << std::endl;
	}

	// TEST 6  N threads editing random nodes in the graph M times each using cpy
	{
		std::cout << std::endl;
