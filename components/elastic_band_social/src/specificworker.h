...
private:
        ...
        QPointF obtener_gaussianas (const QPointF robot_point);
        void distancePointLine( const QPointF &p, const QLineF &line, float &distance, QPointF &intersectPnt);
        QVector2D calculateSocialForce(QVector2D s_force_total, vector<Node> person_list, QPointF p);
        // Grid
        Grid grid;
