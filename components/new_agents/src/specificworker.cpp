...
bool SpecificWorker::setParams(std::unordered_map<std::string, std::string>& params)
{
	try
	{
		agent_name = params["agent_name"];
    	agent_id = stoi(params["agent_id"]);
    	dsr_input_file = params["dsr_input_file"];
   	 	dsr_output_path = params["dsr_output_path"];
		dsr_write_to_file = params["dsr_write_to_file"] == "true";
    	this->Period = stoi(params["period"]);
		tree_view = (params["tree_view"] == "true") ? DSR::DSRViewer::view::tree : 0;
		graph_view = (params["graph_view"] == "true") ? DSR::DSRViewer::view::graph : 0;
		qscene_2d_view = (params["2d_view"] == "true") ? DSR::DSRViewer::view::scene : 0;
		osg_3d_view = (params["3d_view"] == "true") ? DSR::DSRViewer::view::osg : 0;
	}
...
void SpecificWorker::initialize(int period)
{
    ...
    else
    {
        ...
        // Compute max Id in G
        get_max_id_from_G();
        std::cout << "Initialize" << QThread::currentThreadId() << " " << std::this_thread::get_id() << std::endl;


        /*compute_thread = new QThread(this);
        timer = std::make_unique<QTimer>(nullptr);
        timer->callOnTimeout([this]() {
            std::cout << "Timeout" << QThread::currentThreadId() << " " << std::this_thread::get_id() << std::endl;
            compute();
        });
        timer->setInterval(Period);
        timer->moveToThread(compute_thread);
        compute_thread->start();*/

        computeThread = std::make_unique<ComputeThread>([this]() {
            std::cout << "Timeout" << QThread::currentThreadId() << " " << std::this_thread::get_id() << std::endl;
            compute();
        });
        computeThread->setPeriod(Period);
    }
}

void SpecificWorker::compute()
{
    std::cout << "Compute" << QThread::currentThreadId() << " " << std::this_thread::get_id() << std::endl;

    auto x = G->get_connected_agents();
    std::cout << "---------- CONNECTED AGENTS ----------" << std::endl;
    for (auto & agent: x) {
