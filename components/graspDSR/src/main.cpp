...
/** \mainpage RoboComp::graspDSR
 *
 * \section intro_sec Introduction
 *
 * The graspDSR component...
 *
 ...
 *
 * Just: "${PATH_TO_BINARY}/graspDSR --Ice.Config=${PATH_TO_CONFIG_FILE}"
 *
...
class graspDSR : public RoboComp::Application
{
public:
	graspDSR (QString prfx, bool startup_check) { prefix = prfx.toStdString(); this->startup_check_flag=startup_check; }
private:
	void initialize();
	std::string prefix;
	TuplePrx tprx;
...
int ::graspDSR::run(int argc, char* argv[])
{
#ifdef USE_QTGUI
	...
	try
	{
		if (not GenericMonitor::configGetString(communicator(), prefix, "ObjectPoseEstimationRGBDProxy", proxy, ""))
		{
			cout << "[" << PROGRAM_NAME << "]: Can't read configuration for proxy ObjectPoseEstimationRGBDProxy\n";
		}
		objectposeestimationrgbd_proxy = Ice::uncheckedCast<RoboCompObjectPoseEstimationRGBD::ObjectPoseEstimationRGBDPrx>( communicator()->stringToProxy( proxy ) );
	}
	catch(const Ice::Exception& ex)
	{
		cout << "[" << PROGRAM_NAME << "]: Exception creating proxy ObjectPoseEstimationRGBD: " << ex;
		return EXIT_FAILURE;
	}
	rInfo("ObjectPoseEstimationRGBDProxy initialized Ok!");


	tprx = std::make_tuple(dsrgetid_proxy,objectposeestimationrgbd_proxy);
...
int main(int argc, char* argv[])
{
	...
	::graspDSR app(prefix, startup_check_flag);
