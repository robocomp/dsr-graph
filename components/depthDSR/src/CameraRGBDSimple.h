...
struct TDepth
{
     ...
     * @return The data members in a tuple.
     */
    std::tuple<const int&, const int&, const int&, const int&, const int&, const int&, const float&, const ::RoboCompCameraRGBDSimple::DepthType&> ice_tuple() const
    {
        return std::tie(cameraID, width, height, focalx, focaly, alivetime, depthFactor, depth);
    }
};
...
struct StreamReader<::RoboCompCameraRGBDSimple::TDepth, S>
{
    ...
    {
        istr->readAll(v.cameraID, v.width, v.height, v.focalx, v.focaly, v.alivetime, v.depthFactor, v.depth);
    }
};
