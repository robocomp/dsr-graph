...
int ::giraff_dsr::run(int argc, char* argv[])
{
	...
	try
	{
		if (not GenericMonitor::configGetString(communicator(), prefix, "CameraSimple1Proxy", proxy, ""))
		{
			cout << "[" << PROGRAM_NAME << "]: Can't read configuration for proxy CameraSimpleProxy\n";
		}
		camerasimple1_proxy = Ice::uncheckedCast<RoboCompCameraSimple::CameraSimplePrx>( communicator()->stringToProxy( proxy ) );
	}
	catch(const Ice::Exception& ex)
	{
		cout << "[" << PROGRAM_NAME << "]: Exception creating proxy CameraSimple1: " << ex;
		return EXIT_FAILURE;
	}
	rInfo("CameraSimpleProxy1 initialized Ok!");
	...
	try
	{
		if (not GenericMonitor::configGetString(communicator(), prefix, "GiraffProxy", proxy, ""))
		{
			cout << "[" << PROGRAM_NAME << "]: Can't read configuration for proxy GiraffProxy\n";
		}
		giraff_proxy = Ice::uncheckedCast<RoboCompGiraff::GiraffPrx>( communicator()->stringToProxy( proxy ) );
	}
	catch(const Ice::Exception& ex)
	{
		cout << "[" << PROGRAM_NAME << "]: Exception creating proxy Giraff: " << ex;
		return EXIT_FAILURE;
	}
	rInfo("GiraffProxy initialized Ok!");
