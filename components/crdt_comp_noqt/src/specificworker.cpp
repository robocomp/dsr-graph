...
bool SpecificWorker::setParams(RoboCompCommonBehavior::ParameterList params) {
    ...
    nodes_test = stoi(params["nodes"].value);
    return true;
}
...
void SpecificWorker::test_node_random() {
    ...
    int to_move = 0;
    cont=0;
    std::cout << "TEST: "<<nodes_test<<std::endl;
    begin=clock();
    while (cont < nodes_test) {
        try {
            to_move = randomNode(mt);
            if(gcrdt->in(to_move)) {
                float p_x = gcrdt->get_node_attrib_by_name<float>(to_move, "pos_x");
                p_x += 10; //No random here.
    ...
    if(cont == nodes_test) {
        ...
        cout << "Nodes (random ok): " << cont << std::endl;
        cout << "Elapsed time: " << elapsed_secs << std::endl;
        cout << "Nodes / sec: " << cont / elapsed_secs << std::endl;
        cout << "Benchmark: " << 1 / (cont / elapsed_secs)  << " ms. " << std::endl;
        cout << "-------------------------------------" << std::endl;
        cont++;
    }
}
