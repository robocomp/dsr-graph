...
public:
    ...
    CRDT_concurrent_test(const std::shared_ptr<Test_utils> t, int num_threads_, int num_ops_)
            : testutils(t) , num_threads (num_threads_), num_ops (num_ops_){};

    void test(const shared_ptr<CRDT::CRDTGraph>& G);
private:
    std::shared_ptr<Test_utils> testutils;
