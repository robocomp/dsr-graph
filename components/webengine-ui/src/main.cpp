...
/** \mainpage RoboComp::webengine_ui
 *
 * \section intro_sec Introduction
 *
 * The webengine_ui component...
 *
 ...
 *
 * Just: "${PATH_TO_BINARY}/webengine_ui --Ice.Config=${PATH_TO_CONFIG_FILE}"
 *
...
class webengine_ui : public RoboComp::Application
{
public:
	webengine_ui (QString prfx, bool startup_check) { prefix = prfx.toStdString(); this->startup_check_flag=startup_check; }
private:
	void initialize();
	std::string prefix;
	TuplePrx tprx;
...
int ::webengine_ui::run(int argc, char* argv[])
{

    const std::string disable_web_security = "--disable-web-security";
    std::vector<char*> args (argv, argv+argc);
    args.push_back(const_cast<char*>(disable_web_security.c_str()));
    int size = args.size();
#ifdef USE_QTGUI
	QApplication a(size, args.data());  // GUI application
#else
	QCoreApplication a(size, args.data());  // NON-GUI application
#endif
...
int main(int argc, char* argv[])
{
	...
	::webengine_ui app(prefix, startup_check_flag);
