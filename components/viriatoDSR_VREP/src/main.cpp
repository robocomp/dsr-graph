...
/** \mainpage RoboComp::viriatoDSR_VREP
 *
 * \section intro_sec Introduction
 *
 * The viriatoDSR_VREP component...
 *
 ...
 *
 * Just: "${PATH_TO_BINARY}/viriatoDSR_VREP --Ice.Config=${PATH_TO_CONFIG_FILE}"
 *
...
class viriatoDSR_VREP : public RoboComp::Application
{
public:
	viriatoDSR_VREP (QString prfx) { prefix = prfx.toStdString(); }
private:
	void initialize();
	std::string prefix;
	TuplePrx tprx;
...
void ::viriatoDSR_VREP::initialize()
{
...
int ::viriatoDSR_VREP::run(int argc, char* argv[])
{
#ifdef USE_QTGUI
	...
	try
	{
		...
		try
		{
			...
			JoystickAdapterPtr joystickadapterI_ =  std::make_shared <JoystickAdapterI>(worker);
		...
		{
			std::cout << "ERROR Unsubscribing topic: joystickadapter " <<std::endl;
		}
