...
std::ostream&
operator<<(std::ostream& os, DataStorm::SampleEvent sampleType);
/**
 ...
 */
std::ostream&
operator<<(std::ostream& os, const std::vector<DataStorm::SampleEvent>& types);
...
std::ostream& operator<<(std::ostream& os, const DataStorm::Sample <K, V, U> &sample) {
	os << sample.getValue();
	return os;
}
...
public:
     ...
     * that created the sample. It's the name of the writer if a name was
     * explicitly provided on creation of the writer. Otherwise, if no name
     ...
     *
     * This session identifier can be used to retrieve the Ice connection
     * with the node.
     *
...
public:
     ...
     * connects the reader to writers with matching keys. If an empty vector of
     * keys is provided, the reader will connect to all the available writers.
     *
     * @param topic The topic.
     ...
     * keys. If an empty vector of keys is provided, the reader will connect to
     * all the available writers. The writer will only send samples matching the
...
public:
     ...
     * connects the writer to readers with matching keys. If an empty vector of
     * keys is provided, the writer will connect to all the available readers.
     *
     * @param topic The topic.
     * @param keys The keys.
     * @param name The optional writer name.
