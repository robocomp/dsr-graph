...
GraphViewer::GraphViewer(QMainWindow *widget, std::shared_ptr <CRDT::CRDTGraph> G_, int options, view main)
        : QObject() {
    G = G_;
    ...
    if (docks.count(QString("Tree")) == 1) {
        ...
        if (graph_widget) {
            ...
            GraphViewer::connect(
                    ...
                    graph_widget,
                    [=](int value, int id, const std::string &type,
                        QTreeWidgetItem *) { graph_widget->hide_show_node_SLOT(id, value == 2); });
        }
