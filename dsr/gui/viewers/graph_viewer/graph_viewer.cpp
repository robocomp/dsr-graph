...
DSRtoGraphViewer::DSRtoGraphViewer(std::shared_ptr <CRDT::CRDTGraph> G_, QWidget *parent) : AbstractGraphicViewer(
        parent) {
...
void DSRtoGraphViewer::add_or_assign_node_SLOT(int id, const std::string &type) {
    ...
    if (n.has_value()) {
        ...
        if ((posx != gnode->x() or posy != gnode->y()) and gnode != scene.mouseGrabberItem()) {
            qDebug() << __FUNCTION__ << "##### posx " << posx << " != gnode->x() " << gnode->x() << " or posy " << posy
                     << " != gnode->y() " << gnode->y();
...
void DSRtoGraphViewer::del_edge_SLOT(const std::int32_t from, const std::int32_t to, const std::string &edge_tag) {
    ...
    try {
        ...
        std::cout << e.what() << " Error  " << __FUNCTION__ << ":" << __LINE__ << std::endl;
    }
