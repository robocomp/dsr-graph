        ...
        try {
            ...
            if (n.has_value()) {
                ...
                if (lAngles.has_value() and lDists.has_value()) {
                    ...
                    scene.clear();
                    QPolygonF robot;
                    robot << QPointF(-200, 200) << QPointF(-200, -200) << QPointF(200, -200) << QPointF(200, 200)
                          << QPointF(0, 260);
    ...
    DoRGBDStuff(std::shared_ptr <CRDT::CRDTGraph> graph, CRDT::IDType node_id_) {
        ...
        QObject::connect(graph.get(), &CRDT::CRDTGraph::update_attrs_signal,
                         [&](const std::int32_t &id, const std::map <string, Attrib> &attrs) {
                             ...
                             if (n.has_value())
                                 const auto &lDists =
                                         graph->get_attrib_by_name < std::vector < float >> (n.value(), "rgbd_data");
