...
class DoRTStuff : public QTableWidget {
    Q_OBJECT
public:
    DoRTStuff(std::shared_ptr <CRDT::CRDTGraph> graph_, const CRDT::IDType &from_, const CRDT::IDType &to_,
...
//      qRegisterMetaType<CRDT::AttribsMap>("DSR::Attribs");
        ...
        if (n.has_value() && n2.has_value()) {
            ...
            setWindowTitle(
                    "RT: " + QString::fromStdString(n.value().name()) + "(" + QString::fromStdString(n.value().type()) +
                    ") to " + QString::fromStdString(n2.value().name()) + "(" +
