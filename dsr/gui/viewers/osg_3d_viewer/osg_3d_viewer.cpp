...
osg::ref_ptr <osg::Group> DSRtoOSGViewer::createGraph() {
    ...
    try {
        ...
        std::cout << e.what() << " Error in method " << __FUNCTION__ << " : " << std::endl;
    }
...
void DSRtoOSGViewer::add_or_assign_transform(const CRDT::Node &node, const CRDT::Node &parent) {
    ...
    if (auto res = osg_map.find(std::make_tuple(parent.id(), node.id())); res != osg_map.end()) {
        ...
        throw std::runtime_error(
                "Transform: OSG parent not found for " + parent.name() + "-" + std::to_string(parent.id()));
}

void DSRtoOSGViewer::add_or_assign_box(const CRDT::Node &node, const CRDT::Node &parent) {
    std::cout << __FUNCTION__ << ": node " << node.name() << "-" << node.id() << " Parent: " << parent.id()
              << std::endl;
    try {
        ...
        // Create object
        osg::ref_ptr <osg::Box> box = new osg::Box(QVecToOSGVec(QVec::vec3(0, 0, 0)), width.value(), height.value(),
                                                   depth.value());
        ...
        if (auto res = osg_map.find(std::make_tuple(parent.id(), node.id())); res != osg_map.end()) {
            ...
            throw std::runtime_error(
                    "Transform: OSG parent not found for " + node.name() + "-" + std::to_string(node.id()));
        ...
        std::cout << "Exception at insert_or_assign_box " << e.what() << std::endl;
        throw e;
    }
}
...
void DSRtoOSGViewer::add_or_assign_mesh(const CRDT::Node &node, const CRDT::Node &parent) {
    ...
    osg_mesh->getOrCreateStateSet()->setAttributeAndModes(polygonMode,
...
osg::Vec4 DSRtoOSGViewer::htmlStringToOsgVec4(const std::string &color) {
    ...
    blue[0] = color[5];
    blue[1] = color[6];
    return osg::Vec4(float(red.toInt(&ok, 16)) / 255., float(green.toInt(&ok, 16)) / 255.,
                     float(blue.toInt(&ok, 16)) / 255., 0.f);
}
...
void DSRtoOSGViewer::resizeGL(int width, int height) {
    this->getEventQueue()->windowResize(this->x() * m_scaleX, this->y() * m_scaleY, width * m_scaleX,
                                        height * m_scaleY);
