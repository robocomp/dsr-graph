...
DSRtoGraphicsceneViewer::DSRtoGraphicsceneViewer(std::shared_ptr <CRDT::CRDTGraph> G_, QWidget *parent)
        : AbstractGraphicViewer(parent) {
...
void DSRtoGraphicsceneViewer::add_or_assign_edge_slot(const std::int32_t from, const std::int32_t to,
...
void DSRtoGraphicsceneViewer::get_2d_projection(std::string node_name, std::vector<int> size, QPolygon &polygon,
                                                int &zvalue) {
    ...
    if (rt.has_value()) {
        for (unsigned int i = 0; i < cube_positions.size(); i++) {
            QVec vec = rt.value() * QVec::vec4(size[0] * cube_positions[i][0], size[1] * cube_positions[i][1],
                                               size[2] * cube_positions[i][2], 1.0);
...
void DSRtoGraphicsceneViewer::add_or_assign_mesh(CRDT::Node &node) {
    ...
    qDebug() << "Draw mesh" << QString::fromStdString(node.name()) << "(" << width << "," << height << "," << depth
...
//pose.value().print(QString::fromStdString(node.name()));
        ...
        if (scene_map.find(node.id()) == scene_map.end()) {
            QPixmap pixmap = QPixmap::fromImage(
                    QImage("/home/robocomp/robocomp/components/dsr-graph/etc/person.png")).scaled(600, 300);
...
void DSRtoGraphicsceneViewer::add_or_assign_robot(CRDT::Node &node) {
    ...
    if (pose.has_value()) {
        ...
        scenePolygon->setPos(pose.value().x() - scenePolygon->boundingRect().center().x(),
                             pose.value().z() - scenePolygon->boundingRect().center().y());
...
//pose.value().print(QString::fromStdString(node.value().name()));
        ...
        if (pose.has_value()) {
            item->setPos(pose.value().x() - item->boundingRect().center().x(),
                         pose.value().z() - item->boundingRect().center().y());
            item->setRotation(qRadiansToDegrees(-pose.value().ry()) + 180);
        } else {
            qDebug() << "Error gettion tranformation from person" << QString::fromStdString(node.value().name())
                     << "to world";
        }
...
void DSRtoGraphicsceneViewer::del_node_slot(const std::int32_t id) {
    ...
    if (scene_map.count(id) != 0) {
        ...
        for (std::map < std::string, std::vector < int >> ::iterator it = edge_map.begin(); it != edge_map.end();
        it++)
        {
...
void
DSRtoGraphicsceneViewer::del_edge_slot(const std::int32_t from, const std::int32_t to, const std::string &edge_tag) {
    qDebug() << "********************************";
    qDebug() << __FUNCTION__;
